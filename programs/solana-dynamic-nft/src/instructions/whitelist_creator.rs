...
use crate::{error::MetaplexCoreError, program::SolanaDynamicNft, state::WhitelistedCreators};
...
pub struct WhitelistCreator<'info> {
    ...
    pub this_program: Program<'info, SolanaDynamicNft>,
    // Making sure only the program update authority can add creators to the array
    #[account(constraint = program_data.upgrade_authority_address == Some(payer.key()) @ MetaplexCoreError::NotAuthorized)]
