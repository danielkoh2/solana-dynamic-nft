...
use crate::{state::*, error::MetaplexCoreError};
...
pub struct CreateCollection<'info> {
    #[account(
        mut,
        constraint = whitelisted_creators.contains(&creator.to_account_info()) @ MetaplexCoreError::NotAuthorized,
    )]
    pub creator: Signer<'info>,

    #[account(
        mut,
        constraint = collection.data_is_empty() @ MetaplexCoreError::CollectionAlreadyInitialized,
    )]
