...
describe("solana-dynamic-nft", () => {
  ...
  const WhitelistSeed = "whitelist"
  const CollectionAuthoritySeed = "collection_authority"
  before(async ()=>{
    // Fund accounts
    ...
    collectionAuthorityPda = PublicKey.findProgramAddressSync(
        [Buffer.from(CollectionAuthoritySeed), collection.publicKey.toBuffer()],
        program.programId
    )[0];
    ...
    invalidCollectionAuthorityPda = PublicKey.findProgramAddressSync(
        [Buffer.from(CollectionAuthoritySeed), invalidCollection.publicKey.toBuffer()],
        program.programId
    )[0];
